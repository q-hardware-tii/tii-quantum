"""Core Job class and related helpers.
...
def convert_str_to_job_status(status: str) -> T.Optional[JobStatus]:
    ...
    Returns:
        JobStatus enum value if successful, None if not recognized
    """
    try:
...
class Job:
    ...
    def __init__(
        ...
        self._status: T.Optional[JobStatus] = None
    ...
    def refresh(self) -> JobStatus:
        ...
        Returns:
            The current JobStatus enum value
        """
    ...
    def _wait_for_completion(
        ...
        Returns:
            The final JobStatus enum value
        """
    ...
    def _wait_live(self, interval: float) -> JobStatus:
        ...
        with Live(outer, console=console, vertical_overflow="visible") as live:
            while True:
                status = self.refresh()
                if status != JobStatus.POSTPROCESSING:
                    ui.set(
                        "status",
                        build_status_panel(
                            status.name,
    ...
    def _wait_non_live(self, interval: float, verbose: bool) -> JobStatus:
        ...
        while True:
            ...
            if self._status in (JobStatus.SUCCESS, JobStatus.ERROR):
                if verbose:
