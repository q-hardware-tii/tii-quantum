"""Exception hook management for tii-quantum.

This module provides hooks to gracefully handle ApiError exceptions across
...
def _qibo_threading_excepthook(args: threading.ExceptHookArgs):
    ...
    """
    if isinstance(args.exc_value, ApiError):
...
def install_qibo_error_hooks() -> bool:
    ...
    if shell is not None:
        try:
            shell.set_custom_exc((ApiError,), _ipython_custom_exc_handler)
        except Exception:
            pass
...
def uninstall_qibo_error_hooks() -> bool:
    ...
    the tii-quantum hooks. It's idempotent and returns True if uninstalled
