...
class Client:
    ...
    def check_client_server_qibo_versions(self):
        ...
        client_version = Version(qibo.__version__)

        if client_version < qibo_min_client_version:
            raise RuntimeError(
                f"The tii-quantum requires qibo>={qibo_min_client_version}, "
                f"but local version is {client_version}"
            )

        if client_version < qibo_server_version:
            logger.warning(
        ...
        if this_client_version < min_client_version:
            raise RuntimeError(
                f"This tii-quantum version ({this_client_version}) is no longer "
                f"supported by the server. Please upgrade to "
                f"tii-quantum>={min_client_version}."
            )
    ...
    def run_circuit(
        ...
        Returns:
            Job object for monitoring the job, or None if submission failed
    ...
    def get_job(self, pid: str) -> Job:
        ...
        Returns:
            Job object representing the retrieved job
        """
        job = Job(base_url=self.base_url, headers=self.headers, pid=pid)
        job.refresh()
        return job
    ...
    def delete_job(self, pid: str):
        ...
        """
        Job(base_url=self.base_url, headers=self.headers, pid=pid).delete()
    ...
    def delete_all_jobs(self) -> list[str]:
        ...
        deleted_jobs = ApiRequest.delete(
