...
from tii_quantum.exceptions.errors import ApiError
...
def hooks(monkeypatch):
    ...
    import tii_quantum.exceptions.hooks as hooks_mod
...
def make_api_error(message: str = "boom"):
    return ApiError(status=400, method="GET", url="http://test", message=message)
...
def test_ipython_custom_exc_handler_invokes_renderer(monkeypatch, hooks):
    ...
    hooks._ipython_custom_exc_handler(None, ApiError, err, None)
...
def test_qibo_sys_excepthook_renders_api_errors(monkeypatch, hooks):
    ...
    hooks._qibo_sys_excepthook(ApiError, err, None)
...
def test_install_and_uninstall_hooks_manage_global_state(monkeypatch, hooks):
    ...
    assert shell.calls == [((ApiError,), hooks._ipython_custom_exc_handler)]
...
def test_exceptions_init_auto_install_catches_exception(monkeypatch):
    ...
    import tii_quantum.exceptions as exc_mod
    import tii_quantum.exceptions.hooks as hooks_mod
    ...
    try:
        ...
        sys.modules.pop("tii_quantum.exceptions", None)
        importlib.import_module("tii_quantum.exceptions")
