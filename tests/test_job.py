...
from tii_quantum import JobStatus, exceptions, job
from tii_quantum.job import Job


@pytest.mark.parametrize(
    "status, expected_result",
    [
        ("queueing", JobStatus.QUEUEING),
...
def test_convert_str_to_job_status(status, expected_result):
    result = job.convert_str_to_job_status(status)
...
BASE_JOB_STATUS = JobStatus.SUCCESS
...
class TestJob:
    ...
    def setup_and_teardown(self):
        self.obj = job.Job(FAKE_PID, FAKE_URL)
        yield
    ...
    @pytest.mark.parametrize(
        "status, expected_result",
        [
            (JobStatus.QUEUEING, False),
            (JobStatus.PENDING, False),
            (JobStatus.RUNNING, True),
            (JobStatus.POSTPROCESSING, False),
    ...
    def test_wait_for_completion_verbose(self, monkeypatch, caplog, status):
        ...
        assert job_status == JobStatus[status.upper()]
...
class TestLiveTTYBranch:
    ...
    def setup(self, monkeypatch):
        self.obj = job.Job(FAKE_PID, FAKE_URL, device=FAKE_DEVICE)
        monkeypatch.setattr("tii_quantum.job.USE_RICH_UI", True)
        monkeypatch.setattr("tii_quantum.job.constants.TIMEOUT", 2)
    ...
    def test_live_branch_immediate_success(self, monkeypatch):
        ...
        monkeypatch.setattr("tii_quantum.job.console", fake_console)
...
def _make_job():
    ...
    job = Job(pid="PID1", base_url="http://x", circuit=c.raw, nshots=100)
    return job
...
def _make_multi_register_job():
    ...
    job = Job(pid="PID2", base_url="http://x", circuit=c.raw, nshots=100)
    return job
