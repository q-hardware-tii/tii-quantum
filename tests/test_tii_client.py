...
import tii_quantum.tii_client as qc
from tii_quantum import Job, JobStatus, exceptions, tii_client
from tii_quantum.tii_client import Client

MOD = "tii_quantum.tii_client"
...
class TestQiboClient:
    ...
    def setup_and_teardown(self, monkeypatch):
        ...
        self.client = tii_client.Client(FAKE_TOKEN, FAKE_URL)
        yield
    ...
    def test_init_sends_client_identity_headers(self):
        ...
        assert self.client.headers["x-tii-quantum-version"] == qc.version
    ...
    def test_check_client_server_qibo_versions_raises_assertion_error(
        ...
        expected_message = f"The tii-quantum requires qibo>=0.2.8, but local version is {FAKE_QIBO_VERSION}"
    ...
    def test_get_job(self, monkeypatch):
        ...
        assert result.status() == JobStatus.QUEUEING
...
def test_rejects_old_sdk_version(monkeypatch):
    ...
    monkeypatch.setattr(qc.ApiRequest, "get", staticmethod(lambda *a, **k: _Resp()))

    client = Client(token="t", url="http://x")
    with pytest.raises(RuntimeError, match="tii-quantum"):
